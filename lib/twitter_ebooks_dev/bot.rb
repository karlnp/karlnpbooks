...
module Ebooks
  class Bot
    ...
    def configure
      ...
      needs_stream = [@on_follow, @on_message, @on_favorite, @on_mention, @on_timeline].any? {|e| !e.nil?}
    ...
    def start
      ...
      @stream.on_event(:favorite) do |event|
        next if event[:source][:screen_name] == @username
        log "Fav from @#{event[:source][:screen_name]}"
        @on_favorite.call(event[:source]) if @on_favorite
      end
    ...
    def on_favorite(&b); @on_favorite = b; end
  end
end
